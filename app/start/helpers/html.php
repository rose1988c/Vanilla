...
class Html {
    ...
    public static function select_role($id, $role = USER_CS, $checked = null,$className = null) {
        ...
        foreach ($userroles as $userrole) {
            switch ($userrole){
            	case USER_ADMIN :
            		$roles = array_merge($roles, array(USER_CS => '客服', USER_MANAGER => '网站管理员', USER_ADMIN => '超级管理员'));
            		break;
                case USER_MANAGER :
                    $roles = array_merge($roles, array(USER_CS => '客服'));
                    break;
            }
        }
	 ...
	 *
	 * cyw
	 * 2013-3-28 上午10:56:16
	 * @param unknown_type $id
	 * @param unknown_type $role
	 * @param unknown_type $checked
	 * @param unknown_type $className
	 */
    public static function select_send_vip($id, $checked = null,$className = null) {
        ...
        $roles = UserVipUtils::$USER_VIP_TIME;
...
//				$output = "<span title=\"名人堂高手\" class=\"logo_v3_$size\">&nbsp;</span>";
	...
	public static function treasure_box_buddy($treasure_box, $options = false) {
		if($treasure_box == false) return ;
		if($options == false) $options = array();
		$size = 20;
		if(array_key_exists('size', $options)) {
			$size = $options['size'];
        ...
        $alt = $treasure_box->subject . '的头像';
        if (isset($options['alt'])) {
        	$alt = $options['alt'];
        }
        $output = '<a href="' . Urls::treasure_box($treasure_box->group_id, $treasure_box) . '" title="' . $alt . '"' . $target . '>';
        $output.= '<img src="' . urls::treasure_box_icon($treasure_box, $size) . '"/>';
        $output.= '</a>';
        return $output;
	}
   ...
   public static function label_round_pad_zero($num, $precision = 2, $color = false) {
	...
	public static function nickname_vip_tip($roles, $class = false, $style = false) { //
	    if (UserRolesUtils::is_vip_card($roles)){
	        return '<a ' . $class . ' ' . $style . ' href="' . Urls::absolute("/zt/vip/") . '" target="_blank"><img style="height: 14px; width: 17px;" src="/img/VIP/V.gif" /></a>';
	    }
	    return ;
	}    
    	...
    	if  (is_object($treasure_box)){
            ...
            if ( ($treasure_box->auto_public != 0 && $treasure_box->public_at != 0 && $treasure_box->public_at <= time()) ) {
                ...
                if ($auth->is_logged_in() &&  $auth->id  == $treasure_box->user_id){
                    $showprice = number_format($treasure_box->mpay/100, 2, '.', '');
        ...
        switch ($value) {
        	case 4:
                if (is_array($authority)){ 
                    if (in_array(4, $authority)){
                        $checked =  ' checked="checked" ';
                    }
                }
                $output = '<input id="fans_' . $user_id . '" name="' . $user_id. '[]" ' . $checked . ' type="checkbox" value="4"> &nbsp;&nbsp;<label for="fans_' . $user_id . '">查看铁粉</label>';
            break;
            case 3:
                if (is_array($authority)){ 
	...
	public static function mpay_to_flowers( $mpay ){
		_require(DOC_ROOT . '/lib/services/user_pay.php');
    	return ceil($mpay * 10/PAY_EXCHANGE);
	}
	...
	public static function get_group_diff_vote($object, $prev_object, $field = 'show_total_vote'){

        // 1.落后小于500票显示真实票数
        // 2.落后大于500票到1000票 显示落后500+票
        // 3.落后大于1000票到2000票显示落后1000+票
        // 4.落后大于2000票显示落后2000+票

		if( !is_object($object) || !is_object($prev_object) ) return '';
		$diff_vote = $prev_object->{$field} - $object->{$field};
		if( $diff_vote <= 0 ){
			return '';
		}elseif( $diff_vote <= 500 ){
			return (string)$diff_vote;
		}elseif( $diff_vote > 500 && $diff_vote <= 1000 ){
			return '500+';
		}elseif( $diff_vote > 1000 && $diff_vote <= 2000 ){
			return '1000+';
		}elseif( $diff_vote > 2000 ){
			return '2000+';
		}
	}
	
	public static function show_user_group_votes($group_vote){
		
		$votes = is_object($group_vote) ?  $group_vote->votes : $group_vote;
		
		if( $votes <= 0 ){
			return '';
		}elseif( $votes <= 100 ){
			return (string)$votes;
		}elseif( $votes > 100 && $votes <= 500 ){
			return '100+';
		}elseif( $votes > 500 && $votes <= 1000 ){
			return '500+';
		}elseif( $votes > 1000 && $votes <= 2000 ){
			return '1000+';
		}elseif( $votes > 2000 ){
			return '2000+';
		}
	...
	public static function select_match_school($select_id, $select_name, $selected = FALSE, $class_name = ''){
		$arrays = array(
						'南开大学',
						'天津大学',
						'中国民航大学',
		...
		foreach ($arrays as $value){
			$html.= '<option value="' . $value . '"' . ($selected == $value ? ' selected="selected"' : '') . '>' . $value . '</option>';
		}
    	$html.= '</select>';
    	return $html;
	}
