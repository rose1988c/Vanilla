...
class Urls {
    ...
    public static function _get_domain($schema = true) {
        $site_domain = Config::get ( 'app.url' );
        if($schema) {
            if ($schema === true || !Config::get ( 'app.usehttps' ))
	...
	public static function wap_absolute($path = '/', $schema = 'http') {
		if ($schema instanceof DBObject) $schema = true;
		
		$site_domain = config_get ( 'wap_domain' );
		if($schema) {
			if ($schema === true || !config_get('usehttps')) { 
				$site_domain = "http://$site_domain";
			} else {
			    $site_domain = "{$schema}://$site_domain";
			}
		}
		return $site_domain . $path;
	}
	
	public static function wap_match($match, $path = '/') {
		$site_domain = config_get ( 'wap_domain' );
		if( !is_object($match) )
		{
			return false;
		}
		return "http://" . $site_domain . "/match/" . $match->match_id . $path;
	}
	...
	public static function treasure_box_icon($treasure_box, $size = 20){
		$site_domain = self::_get_domain ();
		$url = "$site_domain/img/group3/box/";
		if ($treasure_box->category == TREASURE_BOX_CATEGORY_NORMAL) {
			$url.= 'normal/';
		}elseif( $treasure_box->category == TREASURE_BOX_CATEGORY_VIDEO ){
			$url.= 'video/';
		}
		switch ($treasure_box->level){
			case TREASURE_BOX_LEVEL_DEFAULT:
				$url.= 'default/';
				break;
			case TREASURE_BOX_LEVEL_TIE:
				$url.= 'tie/';
				break;
			case TREASURE_BOX_LEVEL_JIN:
				$url.= 'jin/';
				break;
		}
		$url.= $size . '.gif';
    ...
    public static function new_channel( $channel, $path = '/' )
	{
		$site_domain = self::_get_domain ();
		if( !is_object($channel) ){
			_require(DOC_ROOT . '/lib/services/channel.php');
			$channel = mcc_get_channel($channel);
		}
		return $site_domain . '/channel/' . $channel->short . $path ;
	}
	...
	public static function channel_article($channel, $article)
	{
		return Urls::new_channel($channel, "/article/{$article->user_id}-{$article->article_id}/");
	}
	
	public static function channel_category($channel, $category, $path = '/')
	{
		return Urls::new_channel($channel, "/category{$category->category_id}" . $path);
	}
	
	public static function channel_tag($channel, $tag)
	{
		return Urls::new_channel($channel, "/tag{$tag->tag_id}/");
	}
